...
def setup_clients(credentials_file):
    ...
    crm_v1 = discovery.build(
        "cloudresourcemanager", "v1", credentials=creds, static_discovery=True
    )
    crm_v2 = discovery.build(
        "cloudresourcemanager", "v2", credentials=creds, static_discovery=True
    )
...
class Cli:
    ...
    def __init__(self, credentials_file, organization, folder, use_org_id, fstree):
        """Inits Cli class.
    ...
    def _get_projects(self):
        ...
        gen = (
            ...
            for p in projs["projects"]
            if "parent" in p
            and p["parent"]["type"] == "organization"
            and p["lifecycleState"] == "ACTIVE"
        )
        for p in gen:
    ...
    def run(self):
            ...
            if "folders" in result:
                for folder in result["folders"]:
                    if not self.fstree:
                        ...
                        f_str = "{}".format(folder["displayName"])
                    tree[f_str] = {}
                    ...
                    if _id in fold_parents:
                        for project in fold_parents[_id]:
                            if not self.fstree:
                                ...
                                proj_name = "{}".format(project["projectId"])
        ...
        if self.organization:
            ...
            try:
                ...
                raise SystemExit(
                    "Could not find organization {}" "".format(self.organization)
                )
            ...
            if not self.fstree:
                ...
                org_name = "{}".format(org["displayName"])
            t[org_name] = {}
        ...
        if self.folder:
            folder = (
                self._crm_v2.folders()
                .get(name="folders/{}".format(self.folder))
                .execute()
            )
            if not self.fstree:
                ...
                fold_name = "{} ({})".format(folder["displayName"], self.folder)
            t[fold_name] = {}
...
def make_fs_tree(d):
    """ Creates filesystem entries for projects and folders
        Folders are f directories, projects are simple empty files
    """
    for k, v in d.items():
        if len(v.keys()) > 0:
            os.mkdir(k)
...
def parse_args():
    ...
    group.add_argument(
        "-o", "--organization", help="organization name to use for search"
    )
    group.add_argument("-f", "--folder", help="folder ID to use for search")
    parser.add_argument(
        "--fstree",
        action="store_true",
        help="create fs dirs and files representing org/folder",
    )
    ...
    if not (args.organization or args.folder):
        parser.print_help()
        print(
            "\nOne of [--organization ORGANIZATION | --folder FOLDER] is " "required.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def main():
    ...
    try:
        cli = Cli(
            args.key_file, args.organization, args.folder, args.use_id, args.fstree
        )
    ...
    try:
        ...
        if e.resp["status"] == "403":
            m = (
                "Permission denied with request to Google API.  Ensure "
